...
//Hangfájokat tölt le, és tesz a mappába: US
// ____________________________________________________________________________
 function getSound_US($word, $folder='./US/') {	//Ha feltöltesz, ./h után / jel
	  ...
	  preg_match('/(\/s\/en-US.*\.mp3)/Uim', $tartalom, $matches); 
	  	
	  //kiveszi a hangfájlt 
	  $hangfajl = file_get_contents('http://dictzone.com'.$matches[0]);
      ...
      return '<a href="'.$folder.$word.'.mp3'.'">HANG-LINK_US</a>';
	  }     
 }
...
// ____________________________________________________________________________
 function getSound_UK($word, $folder='./UK/') {	//Ha feltöltesz, ./h után / jel
...
for($i=1;$i<(count($tomb));$i++){
	$van_szokoz=strpos($tomb[$i]," ");
	if($van_szokoz==false){print ($tomb[$i])."</br>";	  
    print getSound_US($tomb[$i])."</br>";	
	}
}
for($j=1;$j<(count($tomb));$j++){
	$van_szokoz=strpos($tomb[$j]," ");
	if($van_szokoz==false){print ($tomb[$j])."</br>";	  
